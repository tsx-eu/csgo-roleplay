...
//#define DEBUG
#define QUEST_UNIQID	"kill-006"
#define QUEST_NAME      "Guérilla urbaine"
...
public void Q1_Start(int objectiveID, int client) {
	...
	menu.AddItem("", "Tuer des civils sur la place de l'indépendance.", ITEMDRAW_DISABLED);
	menu.AddItem("", "----------------", ITEMDRAW_DISABLED);
...
public bool IsKillEligible(int attacker, int victim, const char weapon[64]) {
	...
	if( attacker == victim )
		return false;
	if( !rp_IsTutorialOver(victim) || rp_IsClientNew(victim) )
		return false;
	...
	char tmp[64];
	rp_GetZoneData(rp_GetPlayerZone(victim), zone_type_name, tmp, sizeof(tmp));
