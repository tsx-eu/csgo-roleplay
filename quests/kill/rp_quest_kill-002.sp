...
#define QUEST_UNIQID	"kill-002"
#define QUEST_NAME      "Criminels de la mafia"
...
public bool fwdCanStart(int client) {
	...
	int count = 0;
	for (int i = 1; i <= MaxClients; i++) {
		if( IsValidClient(i) && !rp_GetClientBool(i, b_IsAFK) && rp_GetClientJobID(i) == 91 )
			count++;
	}
	return (count>=5);
}
public void Q1_Start(int objectiveID, int client) {
	...
	menu.AddItem("", "Tuer des mafieux après qu'il ai commis un délis.", ITEMDRAW_DISABLED);
	menu.AddItem("", "----------------", ITEMDRAW_DISABLED);
...
public bool IsKillEligible(int attacker, int victim, const char weapon[64]) {
	...
	if( rp_GetClientJobID(victim) == 91 && rp_GetClientBool(victim, b_MaySteal) == false )
		return true;
...
int getNearestEligible(int client) {
	...
	for (int i = 1; i <= MaxClients; i++) {
		if( IsValidClient(i) && IsPlayerAlive(i) && IsKillEligible(client, i, "") ) {
