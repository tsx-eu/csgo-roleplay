...
public void QUERY_SetMaireName(Handle owner, Handle handle, const char[] error, any client) {
...
void Draw_Mairie_Candidate(int client, int target, int arg) {
	...
	if( target == 0 ) {
		SQL_TQuery(rp_GetDatabase(), QUERY_MairieCandidate, "SELECT `id`, `name`, M.`steamid`, SUM(CASE WHEN `target` IS NULL THEN 0 ELSE 1 END) AS `cpt` FROM `rp_maire` M INNER JOIN `rp_users` U ON U.`steamid`=M.`steamid` INNER JOIN `rp_maire_vote` V ON V.`target`=M.`id` GROUP BY M.`id` ORDER BY RAND();", client);
		return;
	}
	else if( target == -1 ) {
		if( arg == 0 ) {
